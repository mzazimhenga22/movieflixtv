...
import 'package:movie_app/tv_homescreen.dart';
...
class AnimatedBorderBox extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final colors = (index % 2 == 0) ? [accentColor, Colors.purple] : [Colors.purple, accentColor];
...
class _ProfileSelectionScreenState extends State<ProfileSelectionScreen> {
      ...
      if (user == null) {
        ...
        if (mounted) {
          Navigator.pushReplacement(
              context, MaterialPageRoute(builder: (_) => const TVHomeScreen()));
        }
        return;
      }
  ...
  Future<void> _refreshProfiles() async {
    ...
    try {
      ...
      debugPrint('🔄 Refreshed ${profiles.length} profiles: ${profiles.map((p) => p['name'])}');
      ...
      try {
        ...
        debugPrint('🔄 Fallback: Refreshed ${profiles.length} profiles from local database');
        _profiles = List.from(profiles);
        _profilesController.add(_profiles);
        setState(() {});
      } catch (localError) {
                ...
                try {
                  ...
                  if (avatarUrl.isEmpty) {
                    ...
                    final availableAvatars =
                        defaultAvatars.where((a) => !usedAvatars.contains(a)).toList();
                    avatarUrl = availableAvatars.isNotEmpty
                        ? availableAvatars[Random().nextInt(availableAvatars.length)]
                        : defaultAvatars[Random().nextInt(defaultAvatars.length)];
                    backgroundUrl =
                        defaultBackgrounds[Random().nextInt(defaultBackgrounds.length)];
                  } else {
  ...
  Future<void> _deleteProfile(Map<String, dynamic> profile) async {
    ...
    try {
      ...
      scaffoldMessenger
          .showSnackBar(const SnackBar(content: Text('Error deleting profile')));
    }
  }
            ...
            TextButton(
              ...
              onPressed: () {
                ...
                if (pinController.text.trim() == profile['pin']) {
                  ...
                  Navigator.pop(context);
                  ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text("Incorrect PIN for profile '${profile['name']}'.")));
                }
              },
            ),
          ],
  ...
  void _selectProfile(Map<String, dynamic> profile) {
    debugPrint('🚀 Navigating to TV home screen with profile: ${profile['name']}');
      ...
      Navigator.pushReplacement(
        ...
        PageRouteBuilder(
          ...
          transitionsBuilder: (context, animation, secondaryAnimation, child) {
            const begin = 0.0;
            const end = 1.0;
            final scaleTween = Tween(begin: begin, end: end).chain(CurveTween(curve: Curves.easeInOut));
    ...
    return GestureDetector(
      ...
      child: Container(
        decoration: const BoxDecoration(
          ...
          border: Border.fromBorderSide(BorderSide(color: Color.fromRGBO(255, 255, 255, 0.2))),
        ),
...
child: ClipRRect(
  ...
  child: BackdropFilter(
    filter: ImageFilter.blur(sigmaX: 10, sigmaY: 10), // Now recognized with dart:ui import
    child: Container(
      decoration: const BoxDecoration(
        color: Color.fromRGBO(255, 255, 255, 0.1),
        borderRadius: BorderRadius.all(Radius.circular(12)),
        border: Border.fromBorderSide(BorderSide(color: Color.fromRGBO(255, 255, 255, 0.2))),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (locked) const Icon(Icons.lock, color: Colors.white, size: 20),
                    if (isEditing)
                      Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
