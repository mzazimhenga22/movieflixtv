...
class MovieDetailScreenState extends State<MovieDetailScreen> {
    ...
    if (status.isGranted) {
      ...
      await showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          title: const Text("Permission Required", style: TextStyle(color: Colors.white, fontSize: 24)),
          content: const Text(
          ...
          actions: [
            TextButton(
              ...
              child: Text("Cancel",
                  style: TextStyle(color: Provider.of<SettingsProvider>(context, listen: false).accentColor, fontSize: 18)),
            ),
            TextButton(
              onPressed: () async {
  ...
  Future<void> _downloadMovie(Map<String, dynamic> details, {required String resolution, required bool subtitles}) async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Unable to start download. Please try again later.", style: TextStyle(fontSize: 18))),
      );
      return;
    }
    ...
    if (downloadUrl == null || downloadUrl.isEmpty) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Download unavailable at this time.", style: TextStyle(fontSize: 18))),
      );
      return;
    }
    ...
    if (await _requestStoragePermission()) {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Download started (Task ID: $taskId)", style: TextStyle(fontSize: 18))),
      );
    } else {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Storage permission is required to download.", style: TextStyle(fontSize: 18))),
      );
    }
  }
      ...
      builder: (context) {
        return _RatingDialog(
          ...
          onSubmit: () {
            if (!mounted) return;
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text("Rating submitted: $rating", style: TextStyle(fontSize: 18))),
            );
          },
        );
      },
  ...
  Future<void> _launchStreamingPlayer(
    ...
    try {
      ...
      if (mounted) {
        Navigator.pop(context);
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Unable to start streaming. Please try again later.", style: TextStyle(fontSize: 18))),
        );
      }
      return;
    }
    ...
    if (streamUrl.isEmpty) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Streaming unavailable at this time.", style: TextStyle(fontSize: 18))),
        );
      }
      return;
    }
          ...
          child: Padding(
            ...
            child: Wrap(
              ...
              children: (details['tags'] as List)
                  .map((tag) => Chip(
                        label: Text(tag.toString(), style: const TextStyle(color: Colors.white, fontSize: 18)),
    ...
    if (isLoading) {
      return Padding(
        ...
        child: Shimmer.fromColors(
          baseColor: Colors.grey[800]!,
          highlightColor: Colors.grey[600]!,
          child: Container(width: 150, height: 24, color: Colors.grey[800]),
        ),
      );
        ...
        child: Column(
          ...
          children: [
            Shimmer.fromColors(
              baseColor: Colors.grey[800]!,
              highlightColor: Colors.grey[600]!,
              child: Container(width: 120, height: 28, color: Colors.grey[800]),
            ),
              ...
              children: [
                ...
                Wrap(
                  ...
                  children: (details['cast'] as List)
                      ...
                      .entries
                      .map((entry) => Chip(
                            label: Text(entry.value.toString(), style: const TextStyle(color: Colors.white, fontSize: 18)),
        ...
        child: Column(
          ...
          children: [
            ...
            Shimmer.fromColors(
              baseColor: Colors.grey[800]!,
              highlightColor: Colors.grey[600]!,
              child: Container(width: double.infinity, height: 20, color: Colors.grey[800]),
            ),
          ],
        ),
      );
          ...
          CustomScrollView(
            slivers: [
              SliverAppBar(
                ...
                pinned: true,
                backgroundColor: Colors.black87,
                title: Text(details['title'] ?? details['name'] ?? '', style: const TextStyle(fontSize: 28)),
                ...
                delegate: SliverChildBuilderDelegate(
                  (context, index) {
                    ...
                    return FocusScope(
                      child: Focus(
                        canRequestFocus: true,
                        child: RepaintBoundary(child: content[index]),
                      ),
                    );
                  },
    ...
    return Positioned(
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            IconButton(icon: const Icon(Icons.share, color: Colors.white, size: 30), onPressed: onShare),
            IconButton(icon: const Icon(Icons.add, color: Colors.white, size: 30), onPressed: onAddToList),
            IconButton(icon: const Icon(Icons.download, color: Colors.white, size: 30), onPressed: onDownload),
            IconButton(icon: const Icon(Icons.star, color: Colors.white, size: 30), onPressed: onRate),
          ],
        ),
      ),
    );
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text("Select Resolution:", style: TextStyle(color: Colors.white, fontSize: 18)),
          DropdownButton<String>(
            value: _resolution,
            dropdownColor: Colors.black87,
            items: const [
              DropdownMenuItem(value: "480p", child: Text("480p", style: TextStyle(color: Colors.white, fontSize: 18))),
              DropdownMenuItem(value: "720p", child: Text("720p", style: TextStyle(color: Colors.white, fontSize: 18))),
              DropdownMenuItem(value: "1080p", child: Text("1080p", style: TextStyle(color: Colors.white, fontSize: 18))),
            ],
          ...
          Row(
            children: [
              const Text("Enable Subtitles:", style: TextStyle(color: Colors.white, fontSize: 18)),
          ...
          Center(
            child: ElevatedButton(
              ...
              child: const Text("Start Download", style: TextStyle(color: Colors.black, fontSize: 18)),
            ),
          ),
        ],
      ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 20),
          const Text("Select Resolution:", style: TextStyle(fontSize: 18, color: Colors.white)),
          DropdownButton<String>(
            value: _resolution,
            dropdownColor: Colors.black87,
            items: const [
          ...
          Row(
            children: [
              const Text("Enable Subtitles:", style: TextStyle(fontSize: 18, color: Colors.white)),
              Switch(value: _subtitles, activeColor: settings.accentColor, onChanged: (value) => setState(() => _subtitles = value)),
            ],
          ),
          const Spacer(),
          Center(
            child: ElevatedButton(
              ...
              child: const Text("Play Now", style: TextStyle(color: Colors.black, fontSize: 18)),
            ),
          ),
          const SizedBox(height: 20),
        ],
...
class _RatingDialogState extends State<_RatingDialog> {
  ...
  Widget build(BuildContext context) {
    final settings = Provider.of<SettingsProvider>(context);
    return AlertDialog(
      title: Text('Rate ${widget.title}', style: const TextStyle(fontSize: 24, color: Colors.white)),
      content: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          const Text("Select rating:", style: TextStyle(fontSize: 18, color: Colors.white70)),
          Slider(
            value: _rating,
            min: 1.0,
            max: 5.0,
      ...
      actions: [
        TextButton(onPressed: () => Navigator.pop(context), child: const Text("Cancel", style: TextStyle(fontSize: 18, color: Colors.white))),
        ElevatedButton(
          style: ElevatedButton.styleFrom(backgroundColor: settings.accentColor),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            const Text("Preparing your content...", style: TextStyle(color: Colors.white, fontSize: 20), textAlign: TextAlign.center),
...
class TVShowEpisodesSectionState extends State<TVShowEpisodesSection> {
  ...
  Future<void> _fetchEpisodes(int seasonNumber) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Unable to load episodes. Please try again later.', style: TextStyle(fontSize: 18))),
      );
    }
  }
                    ...
                    DropdownButton<int>(
                      ...
                      items: widget.seasons
                          .map<DropdownMenuItem<int>>((season) => DropdownMenuItem(
                                ...
                                child: Text('Season ${season['season_number'] ?? 0}', style: const TextStyle(fontSize: 18)),
                              ))
                          .toList(),
                      onChanged: (value) {
  ...
  Widget _buildEpisodesList() {
    ...
    if (episodes.isEmpty && !_isLoading) {
      return const Padding(
        ...
        child: Text('No episodes available.', style: TextStyle(color: Colors.white70, fontSize: 18)),
      );
    }
            ...
            ClipRRect(
              ...
              child: stillPath != null
                  ...
                  : Container(
                      ...
                      height: 90,
                      color: Colors.grey,
                      child: Icon(Icons.tv, color: settings.accentColor, size: 40),
                    ),
            ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            CircularProgressIndicator(color: settings.accentColor),
            const SizedBox(height: 20),
            const Text("Preparing your episode...", style: TextStyle(color: Colors.white, fontSize: 20), textAlign: TextAlign.center),
